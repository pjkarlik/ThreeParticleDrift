...
export default class Render {
  ...
  makeParticle = (mx, my, mz, type, size) => {
    ...
    const cRed = type ? particleColor : Math.sin(particleColor * 10.0 - 6.0 * Math.PI / 180.0);
    const cGreen = type ? particleColor : Math.sin(particleColor * 8.0 - 5.0);
    const cBlue = type ? particleColor : Math.cos(particleColor * 6.0 - 4.0);

    sphere.material.color.setRGB(cRed, cRed ,cRed);
